...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 浏览器系统通知（右下角弹窗）。设为静音通知，由 WebAudio 统一发出专属音效，杜绝系统与网页双重音效冲突。 */
    function systemNotify(title, body) {
      try {
        ...
        if (N.permission === 'granted') {
          new N(title || '任务完成', { body: body || '', tag: 'dsh-task-done', silent: true })
          return true
        }
        if (N.permission === 'default') {
          N.requestPermission().then((p) => {
            if (p === 'granted') new N(title || '任务完成', { body: body || '', tag: 'dsh-task-done', silent: true })
          }).catch(() => {})
        }
        return false
