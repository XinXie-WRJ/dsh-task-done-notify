...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '}',
      '.dark .dsh-notify-card-title,',
      "[data-theme='dark'] .dsh-notify-card-title,",
      "[data-color-mode='dark'] .dsh-notify-card-title,",
      '@media (prefers-color-scheme: dark) {',
      "  :root:not([data-theme='light']):not(.light) .dsh-notify-card-title {",
      ...
      '  user-select: none;',
      '}',
      '.dsh-sound-item:hover {',
      '  border-color: rgba(16, 185, 129, 0.5);',
      '  transform: translateY(-1px);',
      '  background: rgba(255, 255, 255, 0.85);',
      '}',
      '.dark .dsh-sound-item,',
      "[data-theme='dark'] .dsh-sound-item,",
      "[data-color-mode='dark'] .dsh-sound-item,",
      '@media (prefers-color-scheme: dark) {',
      "  :root:not([data-theme='light']):not(.light) .dsh-sound-item {",
      '    background: rgba(15, 23, 42, 0.5);',
      '    border-color: rgba(255, 255, 255, 0.1);',
      '  }',
      "  :root:not([data-theme='light']):not(.light) .dsh-sound-item:hover {",
      '    background: rgba(15, 23, 42, 0.8);',
      '    border-color: rgba(16, 185, 129, 0.6);',
      '  }',
      '}',
      '.dsh-sound-item.active {',
      '  border-color: #10b981 !important;',
      '  background: rgba(16, 185, 129, 0.08) !important;',
      '  box-shadow: 0 0 0 1px #10b981, 0 4px 12px rgba(16, 185, 129, 0.15);',
      '}',
      '.dark .dsh-sound-item.active,',
      "[data-theme='dark'] .dsh-sound-item.active,",
      "[data-color-mode='dark'] .dsh-sound-item.active,",
      '@media (prefers-color-scheme: dark) {',
      "  :root:not([data-theme='light']):not(.light) .dsh-sound-item.active {",
      ...
      '}',
      '.dsh-volume-slider::-webkit-slider-thumb {',
      '  -webkit-appearance: none;',
      '  appearance: none;',
      '  width: 18px;',
      '  height: 18px;',
      ...
      '  transition: transform 0.15s ease;',
      '}',
      '.dsh-volume-slider::-webkit-slider-thumb:hover {',
      ...
      '}',
      '.dark .dsh-modern-btn,',
      "[data-theme='dark'] .dsh-modern-btn,",
      "[data-color-mode='dark'] .dsh-modern-btn,",
      '@media (prefers-color-scheme: dark) {',
      "  :root:not([data-theme='light']):not(.light) .dsh-modern-btn {",
      '    background: rgba(15, 23, 42, 0.7);',
      '    border-color: rgba(255, 255, 255, 0.15);',
      '  }',
      "  :root:not([data-theme='light']):not(.light) .dsh-modern-btn:hover {",
      ...
      '}',
      '.dsh-modern-input:focus {',
      '  border-color: #10b981;',
      '  box-shadow: 0 0 0 2px rgba(16, 185, 129, 0.2);',
      '}',
      '.dark .dsh-modern-input,',
      "[data-theme='dark'] .dsh-modern-input,",
      "[data-color-mode='dark'] .dsh-modern-input,",
      '@media (prefers-color-scheme: dark) {',
      "  :root:not([data-theme='light']):not(.light) .dsh-modern-input {",
      '    background: rgba(15, 23, 42, 0.7);',
      '    border-color: rgba(255, 255, 255, 0.15);',
      '  }',
      '}',
    ...
    function apply(ctx) {
        ...
        (props) => {
          ...
          react.useEffect(() => {
            ...
            const poll = async () => {
              ...
              // 自愈保护：如果 Host 返回 unread > 0 但 recent 为空（僵尸状态），立即向 Host 发 clear 自愈
              if (rawUnread > 0 && rawRecent.length === 0) {
                rawUnread = 0
              ...
              // 🌟 核心优化：只要网页当前可见（!document.hidden），且用户正处在已完成的会话里，0ms 静默直消已读，绝对不弹角标
              if (isPageVisible && cur && rawRecent.some((rec) => rec.sessionId === cur)) {
                ...
                if (rawRecent.length === 0) rawUnread = 0
              }
          ...
          // 页面关闭/卸载时：向 Host 发送离开信号，确保清理任何可能残留的任务栏角标
          react.useEffect(() => {
            const onLeave = () => {
              try {
                if (typeof navigator !== 'undefined' && navigator.sendBeacon) {
                  navigator.sendBeacon('/dsh-task-done/client-leave')
                } else if (typeof fetch === 'function') {
                  fetch('/dsh-task-done/client-leave', { method: 'POST', keepalive: true }).catch(() => {})
                }
              } catch (e) {}
            }
            if (typeof window !== 'undefined') {
          ...
          const consumeSession = (sid) => {
            ...
            // 本地即时乐观响应
            if (nextUnread <= 0 || nextRecent.length === 0 || curRecent.length <= 1) {
              setExiting(true)
              setPopOpen(false)
            ...
            // 同步 Host 状态：如果队列已空或只剩 1 个，直接调用 clear 彻底清零 Host，杜绝僵尸未读
            if (curRecent.length <= 1 || !sid) {
              post('/dsh-task-done/clear').catch(() => {})
            } else {
              post('/dsh-task-done/consume', { sessionId: sid }).catch(() => {})
            }
          }
          ...
          const handleDocClick = (e) => {
            ...
            if (isBadge) {
              ...
              if (count <= 1) {
                ...
                consumeSession(sid) // 0ms 启动 350ms 丝滑退场动效并同步清除 Host
                if (sid) {
      ...
      function TaskDoneNotifySettingsSection() {
        ...
        const [saveState, setSaveState] = react.useState('synced') // 'synced' | 'saving'
        ...
        const persist = (next) => {
          ...
          saveTimerRef.current = setTimeout(() => {
            post('/dsh-task-done/settings', next).then((val) => {
              if (val && typeof val === 'object') setSettings((prev) => ({ ...prev, ...val }))
              setSaveState('synced')
            }).catch(() => { setSaveState('synced') })
          }, 250)
        }
        ...
        const selectPreset = (type) => {
          ...
          playCustomChime(type, settings.volume)
        }

        const soundPresets = [
          { id: 'crystal', name: '💎 晶莹水滴', desc: '经典空灵上升双音，轻快悦耳' },
          { id: 'chord', name: '🎵 治愈和弦', desc: 'Do-Mi-Sol 上升三和弦，治愈感十足' },
          { id: 'gentle', name: '🍃 柔和舒缓', desc: 'Sol-Mi 柔和敲门双音，低调办公' },
          { id: 'cyber', name: '⚡ 科技未来', desc: '极客未来扫频脉冲，科技感拉满' },
        ]

        const testSound = (type, e) => {
          if (e) e.stopPropagation()
          playCustomChime(type || settings.soundType || 'crystal', settings.volume)
        }
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              'div',
              { className: 'dsh-autosave-badge' },
              saveState === 'saving' ? '⟳ 正在同步云端配置…' : '✓ 配置已自动保存并实时同步'
            ),
          ),
          // 卡片 1: 基础通知与 Windows Toast 弹窗
          react.createElement(
            'div',
            ...
            react.createElement(
              ...
              react.createElement(
                ...
                react.createElement('div', { className: 'dsh-notify-row-label' }, '启用任务完成感知'),
                react.createElement('div', { className: 'dsh-notify-row-desc' }, '总开关：检测 Agent 状态转换并驱动浮标、角标与提示'),
              ),
              react.createElement(
                'label',
            ...
            react.createElement(
              ...
              react.createElement(
                'div',
                { className: 'dsh-notify-row-info' },
                react.createElement('div', { className: 'dsh-notify-row-label' }, 'Windows 10/11 原生 Toast 弹窗'),
              ...
              react.createElement(
                ...
                react.createElement(
                  'button',
                  { className: 'dsh-modern-btn', onClick: testWindowsToast, title: '向 Windows 发送一条测试横幅' },
                  '✨ 立即测试弹窗',
                ),
                react.createElement(
                  'label',
          ...
          react.createElement(
            'div',
            { className: 'dsh-notify-card' },
            react.createElement('div', { className: 'dsh-notify-card-title' }, '🔊 提示音效风格与音量'),
            react.createElement(
              'div',
              { className: 'dsh-notify-row' },
              react.createElement(
                'div',
                { className: 'dsh-notify-row-info' },
                react.createElement('div', { className: 'dsh-notify-row-label' }, '声音提示开关'),
            ...
            react.createElement(
              ...
              react.createElement(
                'div',
                { className: 'dsh-notify-row-info' },
                react.createElement('div', { className: 'dsh-notify-row-label' }, '提示音量大小'),
                react.createElement('div', { className: 'dsh-notify-row-desc' }, '调整合成提示音的输出响度大小'),
              ),
              react.createElement(
                'div',
                { className: 'dsh-volume-box' },
                react.createElement('input', {
                  type: 'range',
                  className: 'dsh-volume-slider',
                  min: 0,
                  max: 100,
                  step: 5,
                ...
                react.createElement('span', { className: 'dsh-volume-val' }, (typeof settings.volume === 'number' ? settings.volume : 80) + '%'),
              ),
            ),
            // 音效卡片网格
            react.createElement(
              ...
              react.createElement('div', { className: 'dsh-notify-row-label' }, '音效预设方案'),
              react.createElement(
                'div',
                { className: 'dsh-sound-grid' },
                ...soundPresets.map((preset) => {
                  const isActive = (settings.soundType || 'crystal') === preset.id
                  return react.createElement(
                    'div',
                    {
                      key: preset.id,
                      className: 'dsh-sound-item' + (isActive ? ' active' : ''),
                      onClick: () => selectPreset(preset.id),
                    },
                    react.createElement(
                      'div',
                      { className: 'dsh-sound-header' },
                      react.createElement('span', { className: 'dsh-sound-name' }, preset.name),
                      react.createElement(
                        'span',
                        {
                          className: 'dsh-sound-play',
                          onClick: (e) => testSound(preset.id, e),
                          title: '试听该音效',
                        },
                        '▶ 试听',
                      ),
                    ),
                    react.createElement('span', { className: 'dsh-sound-desc' }, preset.desc),
                  )
                }),
              ),
            ),
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              react.createElement(
                ...
                react.createElement('div', { className: 'dsh-notify-row-desc' }, '时段内自动静音且不弹 Windows Toast，仅保留应用内静默角标'),
                react.createElement(
                  'div',
                  { className: 'dsh-chip-row' },
                  react.createElement('span', { className: 'dsh-chip', onClick: () => update('quietHours', '23:00-08:00') }, '🌙 夜间 (23:00-08:00)'),
                  react.createElement('span', { className: 'dsh-chip', onClick: () => update('quietHours', '12:00-14:00') }, '☕ 午休 (12:00-14:00)'),
                  react.createElement('span', { className: 'dsh-chip', onClick: () => update('quietHours', '23:00-08:00,12:00-14:00') }, '🛌 夜间 + 午休'),
                  react.createElement('span', { className: 'dsh-chip', onClick: () => update('quietHours', '') }, '🚫 清空'),
                ),
              ),
              react.createElement('input', {
                type: 'text',
                className: 'dsh-modern-input',
                style: { width: '220px' },
