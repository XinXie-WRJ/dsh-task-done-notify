...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '  animation: dshTaskBadgeOut 0.22s cubic-bezier(0.4, 0, 0.2, 1) both !important;',
      '  pointer-events: none !important;',
      '}',
      '.dsh-task-pop.exiting {',
      '  animation: dshTaskPopOut 0.2s cubic-bezier(0.4, 0, 0.2, 1) both !important;',
      '  pointer-events: none !important;',
      '}',
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 消费单个完成会话：即刻本地乐观更新（0ms 响应）+ 优雅退场动画
          ...
          const handleDocClick = (e) => {
            ...
            if (isBadge) {
              ...
              if (count <= 1) {
                ...
                if (sid) {
                  consumeSession(sid) // 0ms 乐观响应 + 优雅退场动画
            ...
            if (isPopItem) {
              ...
              if (sid) {
                consumeSession(sid) // 0ms 消费该项并跳转
                openWithFallback(sid)
              } else {
                flashErr('会话 id 缺失')
              }
          ...
          const pop = (popOpen || exiting) && recent.length > 0
            ? react.createElement(
                'div',
