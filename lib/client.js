        ...
        (props) => {
              ...
              if (alive && r && typeof r.unread === 'number') {
                ...
                if (Array.isArray(r.recent)) {
                  ...
                  if (fresh.length > 0) {
                    ...
                    shown.forEach((rec) => notifiedIdsRef.current.add(rec.sessionId + ':' + rec.time))
                    const titles = shown.map((rec) => String(rec.title || '任务已完成')).filter(Boolean)
                    const title = shown.length === 1 ? titles[0] : shown.length + ' 个任务已完成'
                    const body = titles.length > 1 ? titles.slice(0, 2).join('、') : ''
                    const usedOs = systemNotify(title, body)
                    if (!usedOs) playChime() // 无系统通知权限/失败 → 回退 WebAudio
                  }
                }
              }
          ...
          const badge = react.createElement(
            'div',
            {
              ...
              title: recentRef.current.length > 1 ? '点击展开最近完成的任务，选择跳转' : '点击跳转到该会话；手动切换过去也会消失',
            },
            react.createElement('span', { className: 'dot' }),
            errMsg ? errMsg : String(unread) + ' 个任务已完成',
          ...
          const pop = popOpen && recent.length > 0
            ? react.createElement(
                ...
                react.createElement('div', { className: 'pop-title' }, '最近完成的任务'),
                ...recent.map((r) => react.createElement(
                  'div',
                  { key: r.sessionId, className: 'dsh-task-pop-item', 'data-sid': r.sessionId },
                  react.createElement('span', { className: 'pdot' }),
                  react.createElement('span', { className: 'ptext' }, r.title || '任务已完成'),
