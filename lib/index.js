...
export function apply(ctx) {
  ...
  const isDesktop = Boolean((process.versions && process.versions.electron) || (process.execPath && /dsh-desktop|electron/i.test(process.execPath)))
  ...
  const PS_TEMPLATE = (action) => [
    ...
    '',
    isDesktop
      ? `# 桌面端宿主：仅精准操作自身桌面应用窗口（PID 或 dsh-desktop/electron），绝不碰浏览器窗口
$dsh = Get-Process | Where-Object {
    $_.MainWindowHandle -ne 0 -and
    ($_.Id -eq ${currentPid} -or $_.ProcessName -match 'dsh-desktop|electron|tauri')
} | Select-Object -First 1`
      : `# Web 端宿主：仅匹配当前打开 DeepSeek Harness 的浏览器窗口（chrome/msedge/firefox），显式排除桌面端应用！
$dsh = Get-Process | Where-Object {
    $_.MainWindowHandle -ne 0 -and
    $_.ProcessName -notmatch 'dsh-desktop|electron|tauri' -and
    $_.MainWindowTitle -match 'DeepSeek Harness'
} | Select-Object -First 1`,
    '',
  ...
  async function flush() {
    ...
    if (!body) body = '任务已完成'
    unread = recentCompletions.length
  ...
  const handler = async (req, res) => {
    ...
    try {
      if (p === '/dsh-task-done/poll') {
        // 自愈同步：未读数字严格对齐待处理的完成会话个数（Unique Sessions）
        unread = recentCompletions.length
        if (unread === 0) lastFireSessionId = null
        const inQuiet = isQuietTime(settingsValue.quietHours)
        json(res, {
          ok: true,
          value: {
