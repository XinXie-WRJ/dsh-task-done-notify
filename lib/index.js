...
export function apply(ctx) {
  // ---- 3. 设置持久化：enabled / sound / system 写 settings.yaml（GUI 可切）----
  // 用动态 import（失败不致命）：即使 dsh-settings 不可解析，插件主体照常工作，
  // 只是设置开关功能不可用。注册成功后由 DSH settings 文件提供方自动持久化到 settings.yaml。
  const settingsDefaults = { enabled: true, sound: true, system: true }
  let settingsValue = { ...settingsDefaults }
  let settingsReady = false
  const SETTINGS_NS = 'task-done-notify' // settingsNamespace('task-done-notify') 校验后即原字符串
  ;(async () => {
    try {
