...
export function apply(ctx) {
  ...
  const statusMap = new Map() // agentId -> 上次观测到的状态
  ...
  const PS_TEMPLATE = (action) => [
    ...
    "if ($action -eq 'NOTIFY') {",
    AUMID_REG,
    '    # 点击通知 -> AUMID 激活快捷方式：vbs 通用激活脚本（标题聚焦/路径探测/浏览器兜底），lnk 指向 wscript + vbs',
    '    $actDir = "$env:APPDATA\\dsh-task-done"',
    '    New-Item -ItemType Directory -Path $actDir -Force | Out-Null',
    '    $vbsPath = "$actDir\\activate-dsh-desktop.vbs"',
    "    $vbsBody = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('__ACTVBS_B64__'))",
    '    # 版本标记检查：vbs 内容升级时覆盖（无 BOM UTF-8，wscript 兼容）',
    "    $vbsOld = if (Test-Path $vbsPath) { Get-Content $vbsPath -Raw } else { '' }",
    "    if ($vbsOld -notlike '*dsh-task-done activate v2*') {",
    '        $enc = New-Object System.Text.UTF8Encoding($false)',
    '        [IO.File]::WriteAllText($vbsPath, $vbsBody, $enc)',
    '    }',
    '    $lnkPath = "$env:APPDATA\\Microsoft\\Windows\\Start Menu\\Programs\\DeepSeek Harness (DSH).lnk"',
    "    Add-Type -TypeDefinition '__LNKCSC__' -ErrorAction SilentlyContinue",
    '    try {',
    '        if (-not (Test-Path $lnkPath)) {',
    '            $lnkArgs = \'"\' + $vbsPath + \'"\'',
    "            [Lnk]::Create($lnkPath, \"$env:WINDIR\\System32\\wscript.exe\", $lnkArgs, $actDir, '${AUMID}')",
    '        } else {',
    "            [LnkAumid]::Set($lnkPath, '${AUMID}')",
    '        }',
    "    } catch { Write-Output ('LNK_ERR: ' + $_.Exception.Message) }",
    '    try {',
    ...
    "    } catch { Write-Output ('TOAST_ERR: ' + $_.Exception.Message) }",
    '    & $psBadge',
  ...
  function buildScript(action, title, body, count) {
    ...
    return PS_TEMPLATE(action)
      ...
      .replace('__ACTVBS_B64__', Buffer.from(ACTIVATE_VBS, 'utf8').toString('base64'))
      .replace('__LNKCSC__', LNKCSC)
  }
  ...
  // ---- 任务完成检测：轮询根 agent 的 running -> idle 转换 ----
  function checkStatus() {
    const roots = rootAgents()
    const alive = new Set()
    for (const a of roots) {
      alive.add(a.id)
      const prev = statusMap.get(a.id)
      statusMap.set(a.id, a.status)
      if (prev === 'running' && a.status === 'idle') fire(a)
    }
    for (const id of Array.from(statusMap.keys())) {
      if (!alive.has(id)) statusMap.delete(id)
    }
  }
  ...
  // ---- 启动基线：现有 agent 状态只记录，不触发 ----
  for (const a of rootAgents()) statusMap.set(a.id, a.status)
  ctx.effect(() => ctx.interval(checkStatus, 1200), 'dsh-task-done: status polling')
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/dsh-task-done/diag') {
        json(res, { ok: true, value: { unread, pending, roots: rootAgents().length, seen: statusMap.size } })
        return
      }
